  ...
  methods: {
    ...
    async addNewCar() {
      ...
      const payload = {
        ...
        mileage: Number(this.newCar.mileage),
        color: this.newCar.color,
        match: Number(this.newCar.match),
        image: this.newCar.image
      }

      try {
        const response = await axios.post(`${API_URL}/cars`, payload)
        ...
        Swal.fire({
          ...
          text: 'Please make sure JSON Server is running.'
        })
      } finally {
        this.saving = false
      }
    ...
    async deleteCar(id) {
      ...
      try {
        ...
        await Swal.fire({
          title: 'Deleted!',
          text: 'The car was deleted successfully.',
          icon: 'success',
          confirmButtonColor: '#1F6F5B'
        })
      } catch (error) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Delete failed',
          text: 'Could not delete this car.'
        })
      }
    },
    ...
    async editCar(car) {
      ...
      try {
        await axios.patch(`${API_URL}/cars/${car.id}`, { price: newPrice })
        car.price = newPrice

        await Swal.fire({
          title: 'Updated!',
          text: 'The price was updated successfully.',
          icon: 'success',
          confirmButtonColor: '#1F6F5B'
        })
      } catch (error) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Update failed',
          text: 'Could not update the car price.'
        })
      }
    },
